  ...
  process(clk)
    begin
	   if (falling_edge(clk)) then
		    if (loadEn = '1') then
			     s_shiftReg <= dataIn;
			 elsif (rotate = '1') then -- Levar o ultimo bit ao inicio ou final
			 
			    if (dirLeft = '1') then -- esquerda 1 / direita 0
				     s_shiftReg <= s_shiftReg(6 downto 0) & s_shiftReg(7);
			    else
				     s_shiftReg <= s_shiftReg(0) & s_shiftReg(7 downto 1);
				 end if;
			elsif (shArith = '1') then -- SHIFT
			    if (dirLeft = '1') then
				    s_shiftReg <= s_shiftReg(6 downto 0) & '0';
				 else -- direita aritmetica
				    s_shiftReg <= s_shiftReg(7) & s_shiftReg(7 downto 1);
					           -- entra o bit 8 (again) copias os 7 primeiros
				 end if;
         else -- logic shift
             if (dirLeft = '1') then
